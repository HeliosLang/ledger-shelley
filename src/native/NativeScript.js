...
export class NativeScript {
  ...
  static fromCbor(bytes, decodeChild = NativeScript.fromCbor) {
    ...
    switch (tag) {
      ...
      case 1:
        return NativeScript.All(decodeItem((s) => decodeList(s, decodeChild)));
      case 2:
        return NativeScript.Any(decodeItem((s) => decodeList(s, decodeChild)));
      case 3:
   ...
   */
  get nRequired() {
    return /** @type {any} */ (this.isAtLeast() ? this.props.nRequired : None);
  }
  ...
  toCbor() {
    if (this.isSig()) {
      ...
      return encodeTuple([encodeInt(1), encodeDefList(this.props.scripts)]);
    } else if (this.isAny()) {
      return encodeTuple([encodeInt(2), encodeDefList(this.props.scripts)]);
    } else if (this.isAtLeast()) {
      const props = this.props;
